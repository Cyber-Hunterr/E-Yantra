defmodule Task3_algorithm_development do
...
#     IO.inspect matrix_of_sum, label:  "matrix_of_sum"
    ...
    result = Map.new()
    optimal_subsets(array_of_digits,valid_sums,result)
  end

  def optimal_subsets(_,[],result) do
      result
  end
  def optimal_subsets([],valid_sums,result) do
      sum_val = List.first(valid_sums)
      result = Map.put_new(result,sum_val,[])
      new_sums = valid_sums -- [sum_val]
      optimal_subsets([],new_sums,result)
  end
  def optimal_subsets(array_of_digits,valid_sums,result) do
      sum_val = List.first(valid_sums)
      ...
      if tsubsets == [] do
            ...
            new_sums = valid_sums -- [sum_val]
            optimal_subsets(new_digits,new_sums,result)
      else
